...
export class FlameGraphRenderer {
  ...
  render(): void {
    ...
    // Get root value for normalization
    const rootValue = this.#dataProcessor.getData()?.value || 1
    const maxDepth = this.#dataProcessor.getMaxDepth()
    ...
    const { frameColors, frameOpacities } = this.#frameRenderer.renderFrames(
      this.#frames,
      matrix,
      this.#logicalWidth,
      viewportWidth,
